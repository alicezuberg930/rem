...
export const parseChatSocketEvent = (payload: string): ChatSocketEvent => {
  ...
  if (event.type === 'PRESENCE_SNAPSHOT' && Array.isArray(event.onlineUserIds) && event.onlineUserIds.every((userId) => typeof userId === 'string')) {
    return {
      type: 'PRESENCE_SNAPSHOT',
      onlineUserIds: event.onlineUserIds,
    }
  }
  if (event.type === 'PRESENCE_CHANGED' && typeof event.userId === 'string' && typeof event.online === 'boolean') {
    return {

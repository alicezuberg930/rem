...
type ToastOptions = Omit<ToastManagerAddOptions<Record<string, unknown>>, 'title' | 'type'>
type ToastUpdateOptions = ToastManagerUpdateOptions<Record<string, unknown>>
type ToastPromiseResult = string | ToastUpdateOptions | null | undefined
type ToastPromiseState<Value> = ToastPromiseResult | ((value: Value) => ToastPromiseResult)
...
function addToast(
  ...
  return toastManager.add({ ...options, title, type })
}
...
const toast = {
  ...
  message: (title: React.ReactNode, options?: ToastOptions) => addToast(undefined, title, options),
  ...
  promise<Value>(
    promiseOrFactory: Promise<Value> | (() => Promise<Value>),
    options: ToastPromiseOptions<Value>
  ) {
    const promise = typeof promiseOrFactory === 'function' ? promiseOrFactory() : promiseOrFactory
    const handledPromise = toastManager.promise(promise, {
      loading: options.loading,
      success: (value) => resolvePromiseState(options.success, value, 'Completed'),
      error: (error) => resolvePromiseState(options.error, error, 'Something went wrong'),
    })

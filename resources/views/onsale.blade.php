            ...
            @if ($product->percent > 0)
                <div class="disable-select product-item col-6 col-md-4 col-lg-2 mb-4">
                    <div class="card card-product h-100" role="button">
                        ...
                        <a href="/product_detail/{{ $product->ProductsID }}"
                            class="h-100 d-flex flex-column text-decoration-none">
                            <div>
                            ...
                            <div class="p-1">
                                ...
                                <div class="card-text mt-2">
                                    <span
                                        class="product-discount float-left badge bg-primary text-white">-{{ $product->percent }}%</span>
                                    <span class="float-right badge">
                                        <span
                                            class="product-discount-price text-decoration-line-through text-secondary mr-1">{{ number_format($product->price, 0, '.') }}VND</span>';
                                        <?php $discount = $product->price * (1 - doubleval($product->percent / 100)); ?>
                                        <span
                                            class="product-final-price text-info">{{ number_format($discount, 0, '.') }}VND</span>
                                </div>
                            </div>
                        </a>
                    </div>

...
class CustomerController extends Controller
{
    ...
    public function customerReload($current_page)
    {
        ...
        if (session()->has('search') && session()->get('search') != '') {
            $query = User::where('name', 'like', '%' . session()->get('search') . '%');
            $total = $query->count();
        ...
        return view('dynamic_layout.customers_reload', ['Customers' => $Customers, 'total' => $total, 'currentpage' => $current_page])->render();
    }
}
